...
class TraitAllOfTraitSpecific(BaseModel):
    ...
    """
    # data type: Choice
    oneof_schema_1_validator: Optional[Choice] = None
    # data type: Choice
    oneof_schema_2_validator: Optional[Choice] = None
    # data type: TraitAllOfTraitSpecificOneOf
    oneof_schema_3_validator: Optional[TraitAllOfTraitSpecificOneOf] = None
    if TYPE_CHECKING:
        actual_instance: Union[Choice, TraitAllOfTraitSpecificOneOf]
    else:
        actual_instance: Any
    one_of_schemas: List[str] = Field(TRAITALLOFTRAITSPECIFIC_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TraitAllOfTraitSpecific.construct()
        error_messages = []
        match = 0
        # validate data type: Choice
        if not isinstance(v, Choice):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Choice`")
        else:
            match += 1
        # validate data type: Choice
        if not isinstance(v, Choice):
            ...
            match += 1
        # validate data type: TraitAllOfTraitSpecificOneOf
        if not isinstance(v, TraitAllOfTraitSpecificOneOf):
            error_messages.append(f"Error! Input type `{type(v)}` is not `TraitAllOfTraitSpecificOneOf`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TraitAllOfTraitSpecific with oneOf schemas: Choice, TraitAllOfTraitSpecificOneOf. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TraitAllOfTraitSpecific with oneOf schemas: Choice, TraitAllOfTraitSpecificOneOf. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> TraitAllOfTraitSpecific:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> TraitAllOfTraitSpecific:
        """Returns the object represented by the json string"""
        instance = TraitAllOfTraitSpecific.construct()
        error_messages = []
        match = 0

        # deserialize data into Choice
        try:
            instance.actual_instance = Choice.from_json(json_str)
            match += 1
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into TraitAllOfTraitSpecificOneOf
        try:
            instance.actual_instance = TraitAllOfTraitSpecificOneOf.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TraitAllOfTraitSpecific with oneOf schemas: Choice, TraitAllOfTraitSpecificOneOf. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TraitAllOfTraitSpecific with oneOf schemas: Choice, TraitAllOfTraitSpecificOneOf. Details: " + ", ".join(error_messages))
        else:
            return instance
