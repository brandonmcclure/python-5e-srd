...
class OptionSetOneOf2(BaseModel):
    """
    OptionSetOneOf2
    """
    option_set_type: Optional[StrictStr] = Field(None, description="Type of option set; determines other attributes.")
    resource_list: Optional[StrictStr] = Field(None, description="A reference (by URL) to a collection in the database.")
    __properties = ["option_set_type", "resource_list"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionSetOneOf2:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OptionSetOneOf2:
        """Create an instance of OptionSetOneOf2 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionSetOneOf2.parse_obj(obj)

        _obj = OptionSetOneOf2.parse_obj({
