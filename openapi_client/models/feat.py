...
class Feat(BaseModel):
    ...
    prerequisites: Optional[conlist(Prerequisite)] = Field(None, description="An object of APIReferences to ability scores and minimum scores.")
    __properties = ["index", "name", "url", "desc", "prerequisites"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Feat:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Feat:
        """Create an instance of Feat from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Feat.parse_obj(obj)
