...
class ClassLevelClassSpecificAnyOf9(BaseModel):
    ...
    creating_spell_slots: Optional[conlist(ClassLevelClassSpecificAnyOf9CreatingSpellSlotsInner)] = None
    __properties = ["sorcery_points", "metamagic_known", "creating_spell_slots"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClassLevelClassSpecificAnyOf9:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in creating_spell_slots (list)
        _items = []
        if self.creating_spell_slots:
            for _item in self.creating_spell_slots:
                if _item:
                    _items.append(_item.to_dict())
            _dict['creating_spell_slots'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ClassLevelClassSpecificAnyOf9:
        """Create an instance of ClassLevelClassSpecificAnyOf9 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClassLevelClassSpecificAnyOf9.parse_obj(obj)

        _obj = ClassLevelClassSpecificAnyOf9.parse_obj({
            "sorcery_points": obj.get("sorcery_points"),
            "metamagic_known": obj.get("metamagic_known"),
            "creating_spell_slots": [ClassLevelClassSpecificAnyOf9CreatingSpellSlotsInner.from_dict(_item) for _item in obj.get("creating_spell_slots")] if obj.get("creating_spell_slots") is not None else None
        })
        return _obj
