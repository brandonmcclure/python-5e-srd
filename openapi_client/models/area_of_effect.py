...
class AreaOfEffect(BaseModel):
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('sphere', 'cone', 'cylinder', 'line', 'cube'):
            raise ValueError("must be one of enum values ('sphere', 'cone', 'cylinder', 'line', 'cube')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AreaOfEffect:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AreaOfEffect:
        """Create an instance of AreaOfEffect from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AreaOfEffect.parse_obj(obj)

        _obj = AreaOfEffect.parse_obj({
            "size": obj.get("size"),
            "type": obj.get("type")
        })
        return _obj
