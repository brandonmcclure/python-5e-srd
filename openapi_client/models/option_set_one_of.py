...
class OptionSetOneOf(BaseModel):
    """
    OptionSetOneOf
    """
    option_set_type: Optional[StrictStr] = Field(None, description="Type of option set; determines other attributes.")
    options_array: Optional[conlist(Option)] = Field(None, description="Array of options to choose from.")
    __properties = ["option_set_type", "options_array"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionSetOneOf:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in options_array (list)
        _items = []
        if self.options_array:
            for _item in self.options_array:
                if _item:
                    _items.append(_item.to_dict())
            _dict['options_array'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> OptionSetOneOf:
        """Create an instance of OptionSetOneOf from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionSetOneOf.parse_obj(obj)

        _obj = OptionSetOneOf.parse_obj({
            "option_set_type": obj.get("option_set_type"),
            "options_array": [Option.from_dict(_item) for _item in obj.get("options_array")] if obj.get("options_array") is not None else None
        })
        return _obj

from openapi_client.models.option import Option
OptionSetOneOf.update_forward_refs()
