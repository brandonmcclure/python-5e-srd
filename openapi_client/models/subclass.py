...
class Subclass(BaseModel):
    ...
    subclass_flavor: Optional[StrictStr] = Field(None, description="Lore-friendly flavor text for a classes respective subclass.")
    subclass_levels: Optional[StrictStr] = Field(None, description="Resource url that shows the subclass level progression.")
    spells: Optional[conlist(SubclassAllOfSpells)] = None
    __properties = ["index", "name", "url", "desc", "class", "subclass_flavor", "subclass_levels", "spells"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Subclass:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in spells (list)
        _items = []
        if self.spells:
            for _item in self.spells:
                if _item:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Subclass:
        """Create an instance of Subclass from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Subclass.parse_obj(obj)

        _obj = Subclass.parse_obj({
            "index": obj.get("index"),
            "name": obj.get("name"),
            "url": obj.get("url"),
            "desc": obj.get("desc"),
            "var_class": APIReference.from_dict(obj.get("class")) if obj.get("class") is not None else None,
            "subclass_flavor": obj.get("subclass_flavor"),
            "subclass_levels": obj.get("subclass_levels"),
            "spells": [SubclassAllOfSpells.from_dict(_item) for _item in obj.get("spells")] if obj.get("spells") is not None else None
        })
        return _obj
