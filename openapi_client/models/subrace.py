...
class Subrace(BaseModel):
    ...
    desc: Optional[StrictStr] = Field(None, description="Description of the subrace.")
    race: Optional[APIReference] = None
    ability_bonuses: Optional[conlist(AbilityBonus)] = Field(None, description="Additional ability bonuses for the subrace.")
    starting_proficiencies: Optional[conlist(APIReference)] = Field(None, description="Starting proficiencies for all new characters of the subrace.")
    languages: Optional[conlist(APIReference)] = Field(None, description="Starting languages for all new characters of the subrace.")
    language_options: Optional[Choice] = None
    racial_traits: Optional[conlist(APIReference)] = Field(None, description="List of traits that for the subrace.")
    __properties = ["index", "name", "url", "desc", "race", "ability_bonuses", "starting_proficiencies", "languages", "language_options", "racial_traits"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Subrace:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in racial_traits (list)
        _items = []
        if self.racial_traits:
            for _item in self.racial_traits:
                if _item:
                    _items.append(_item.to_dict())
            _dict['racial_traits'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> Subrace:
        """Create an instance of Subrace from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Subrace.parse_obj(obj)

        _obj = Subrace.parse_obj({
            "index": obj.get("index"),
            "name": obj.get("name"),
            "url": obj.get("url"),
            "desc": obj.get("desc"),
            "race": APIReference.from_dict(obj.get("race")) if obj.get("race") is not None else None,
            ...
            "racial_traits": [APIReference.from_dict(_item) for _item in obj.get("racial_traits")] if obj.get("racial_traits") is not None else None
        })
        return _obj
