...
class OptionOneOf5(BaseModel):
    ...
    alignments: Optional[conlist(APIReference)] = Field(None, description="A list of alignments of those who might follow the ideal.")
    __properties = ["option_type", "desc", "alignments"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionOneOf5:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in alignments (list)
        _items = []
        if self.alignments:
            for _item in self.alignments:
                if _item:
                    _items.append(_item.to_dict())
            _dict['alignments'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> OptionOneOf5:
        """Create an instance of OptionOneOf5 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionOneOf5.parse_obj(obj)

        _obj = OptionOneOf5.parse_obj({
            "option_type": obj.get("option_type"),
            "desc": obj.get("desc"),
            "alignments": [APIReference.from_dict(_item) for _item in obj.get("alignments")] if obj.get("alignments") is not None else None
        })
        return _obj
