...
class MonsterAllOfSpeed(BaseModel):
    """
    Speed for a monster determines how fast it can move per turn.
    """
    walk: Optional[StrictStr] = Field(None, description="All creatures have a walking speed, simply called the monster’s speed. Creatures that have no form of ground-based locomotion have a walking speed of 0 feet.")
    burrow: Optional[StrictStr] = Field(None, description="A monster that has a burrowing speed can use that speed to move through sand, earth, mud, or ice. A monster can’t burrow through solid rock unless it has a special trait that allows it to do so.")
    climb: Optional[StrictStr] = Field(None, description="A monster that has a climbing speed can use all or part of its movement to move on vertical surfaces. The monster doesn’t need to spend extra movement to climb.")
    fly: Optional[StrictStr] = Field(None, description="A monster that has a flying speed can use all or part of its movement to fly.")
    swim: Optional[StrictStr] = Field(None, description="A monster that has a swimming speed doesn’t need to spend extra movement to swim.")
    __properties = ["walk", "burrow", "climb", "fly", "swim"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MonsterAllOfSpeed:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MonsterAllOfSpeed:
        """Create an instance of MonsterAllOfSpeed from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MonsterAllOfSpeed.parse_obj(obj)

        _obj = MonsterAllOfSpeed.parse_obj({
            "walk": obj.get("walk"),
