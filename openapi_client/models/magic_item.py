...
class MagicItem(BaseModel):
    ...
    rarity: Optional[MagicItemAllOfRarity] = None
    variants: Optional[conlist(APIReference)] = None
    variant: Optional[StrictBool] = Field(None, description="Whether this is a variant or not")
    __properties = ["index", "name", "url", "desc", "equipment_category", "rarity", "variants", "variant"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MagicItem:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of rarity
        if self.rarity:
            _dict['rarity'] = self.rarity.to_dict()
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MagicItem:
        """Create an instance of MagicItem from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MagicItem.parse_obj(obj)

        _obj = MagicItem.parse_obj({
            "index": obj.get("index"),
            "name": obj.get("name"),
            "url": obj.get("url"),
            "desc": obj.get("desc"),
            "equipment_category": APIReference.from_dict(obj.get("equipment_category")) if obj.get("equipment_category") is not None else None,
            "rarity": MagicItemAllOfRarity.from_dict(obj.get("rarity")) if obj.get("rarity") is not None else None,
            "variants": [APIReference.from_dict(_item) for _item in obj.get("variants")] if obj.get("variants") is not None else None,
