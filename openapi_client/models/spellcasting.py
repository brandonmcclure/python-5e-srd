...
class Spellcasting(BaseModel):
    ...
    spellcasting_ability: Optional[APIReference] = None
    __properties = ["level", "info", "spellcasting_ability"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Spellcasting:
    ...
    def to_dict(self):
        ...
        if self.info:
            ...
            _dict['info'] = _items
        # override the default output from pydantic by calling `to_dict()` of spellcasting_ability
        if self.spellcasting_ability:
            _dict['spellcasting_ability'] = self.spellcasting_ability.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> Spellcasting:
        """Create an instance of Spellcasting from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Spellcasting.parse_obj(obj)

        _obj = Spellcasting.parse_obj({
            "level": obj.get("level"),
            "info": [SpellcastingInfoInner.from_dict(_item) for _item in obj.get("info")] if obj.get("info") is not None else None,
            "spellcasting_ability": APIReference.from_dict(obj.get("spellcasting_ability")) if obj.get("spellcasting_ability") is not None else None
        })
        return _obj
