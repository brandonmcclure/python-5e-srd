...
class DC(BaseModel):
    ...
    success_type: Optional[StrictStr] = Field(None, description="Result of a successful save. Can be \\\"none\\\", \\\"half\\\", or \\\"other\\\"")
    __properties = ["dc_type", "dc_value", "success_type"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DC:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of dc_type
        if self.dc_type:
            _dict['dc_type'] = self.dc_type.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> DC:
        """Create an instance of DC from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DC.parse_obj(obj)

        _obj = DC.parse_obj({
            "dc_type": APIReference.from_dict(obj.get("dc_type")) if obj.get("dc_type") is not None else None,
            "dc_value": obj.get("dc_value"),
            "success_type": obj.get("success_type")
        })
        return _obj
