...
class Multiclassing(BaseModel):
    ...
    proficiencies: Optional[conlist(APIReference)] = Field(None, description="List of proficiencies available when multiclassing.")
    proficiency_choices: Optional[conlist(Choice)] = Field(None, description="List of choices of proficiencies that are given when multiclassing.")
    __properties = ["prerequisites", "prerequisite_options", "proficiencies", "proficiency_choices"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Multiclassing:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in prerequisite_options (list)
        _items = []
        if self.prerequisite_options:
            for _item in self.prerequisite_options:
                if _item:
                    _items.append(_item.to_dict())
            _dict['prerequisite_options'] = _items
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Multiclassing:
        """Create an instance of Multiclassing from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Multiclassing.parse_obj(obj)

        _obj = Multiclassing.parse_obj({
            "prerequisites": [Prerequisite.from_dict(_item) for _item in obj.get("prerequisites")] if obj.get("prerequisites") is not None else None,
            "prerequisite_options": [Choice.from_dict(_item) for _item in obj.get("prerequisite_options")] if obj.get("prerequisite_options") is not None else None,
