...
class EquipmentPack(BaseModel):
    ...
    gear_category: Optional[APIReference] = None
    cost: Optional[Cost] = None
    contents: Optional[conlist(APIReference)] = Field(None, description="The list of adventuring gear in the pack.")
    __properties = ["index", "name", "url", "desc", "equipment_category", "gear_category", "cost", "contents"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> EquipmentPack:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of gear_category
        if self.gear_category:
            _dict['gear_category'] = self.gear_category.to_dict()
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> EquipmentPack:
        """Create an instance of EquipmentPack from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EquipmentPack.parse_obj(obj)

        _obj = EquipmentPack.parse_obj({
            "index": obj.get("index"),
            "name": obj.get("name"),
            "url": obj.get("url"),
            "desc": obj.get("desc"),
            "equipment_category": APIReference.from_dict(obj.get("equipment_category")) if obj.get("equipment_category") is not None else None,
            "gear_category": APIReference.from_dict(obj.get("gear_category")) if obj.get("gear_category") is not None else None,
            "cost": Cost.from_dict(obj.get("cost")) if obj.get("cost") is not None else None,
            "contents": [APIReference.from_dict(_item) for _item in obj.get("contents")] if obj.get("contents") is not None else None
        })
        return _obj
