...
class OptionOneOf9(BaseModel):
    ...
    dc: Optional[DC] = None
    damage: Optional[conlist(Damage)] = Field(None, description="Damage dealt by the breath attack, if any.")
    __properties = ["option_type", "name", "dc", "damage"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionOneOf9:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OptionOneOf9:
        """Create an instance of OptionOneOf9 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionOneOf9.parse_obj(obj)

        _obj = OptionOneOf9.parse_obj({
