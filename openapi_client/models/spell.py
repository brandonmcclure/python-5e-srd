...
class Spell(BaseModel):
    ...
    higher_level: Optional[conlist(StrictStr)] = Field(None, description="List of descriptions for casting the spell at higher levels.")
    range: Optional[StrictStr] = Field(None, description="Range of the spell, usually expressed in feet.")
    components: Optional[conlist(StrictStr)] = Field(None, description="List of shorthand for required components of the spell. V: verbal S: somatic M: material ")
    material: Optional[StrictStr] = Field(None, description="Material component for the spell to be cast.")
    area_of_effect: Optional[AreaOfEffect] = None
    ritual: Optional[StrictBool] = Field(None, description="Determines if a spell can be cast in a 10-min(in-game) ritual.")
    duration: Optional[StrictStr] = Field(None, description="How long the spell effect lasts.")
    concentration: Optional[StrictBool] = Field(None, description="Determines if a spell needs concentration to persist.")
    casting_time: Optional[StrictStr] = Field(None, description="How long it takes for the spell to activate.")
    level: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="Level of the spell.")
    attack_type: Optional[StrictStr] = Field(None, description="Attack type of the spell.")
    damage: Optional[SpellAllOfDamage] = None
    school: Optional[APIReference] = None
    classes: Optional[conlist(APIReference)] = Field(None, description="List of classes that are able to learn the spell.")
    subclasses: Optional[conlist(APIReference)] = Field(None, description="List of subclasses that have access to the spell.")
    __properties = ["index", "name", "url", "desc", "higher_level", "range", "components", "material", "area_of_effect", "ritual", "duration", "concentration", "casting_time", "level", "attack_type", "damage", "school", "classes", "subclasses"]
    ...
    def components_validate_enum(cls, value):
        ...
        for i in value:
            if i not in ('V', 'S', 'M'):
                raise ValueError("each list item must be one of ('V', 'S', 'M')")
        return value
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of area_of_effect
        if self.area_of_effect:
            _dict['area_of_effect'] = self.area_of_effect.to_dict()
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Spell:
        """Create an instance of Spell from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Spell.parse_obj(obj)

        _obj = Spell.parse_obj({
            "index": obj.get("index"),
            "name": obj.get("name"),
            "url": obj.get("url"),
            "desc": obj.get("desc"),
            ...
            "area_of_effect": AreaOfEffect.from_dict(obj.get("area_of_effect")) if obj.get("area_of_effect") is not None else None,
            ...
            "damage": SpellAllOfDamage.from_dict(obj.get("damage")) if obj.get("damage") is not None else None,
            "school": APIReference.from_dict(obj.get("school")) if obj.get("school") is not None else None,
            "classes": [APIReference.from_dict(_item) for _item in obj.get("classes")] if obj.get("classes") is not None else None,
