...
class APIReference(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> APIReference:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> APIReference:
        """Create an instance of APIReference from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return APIReference.parse_obj(obj)

        _obj = APIReference.parse_obj({
            "index": obj.get("index"),
            "name": obj.get("name"),
            "url": obj.get("url")
        })
