...
class EquipmentCategory(BaseModel):
    ...
    equipment: Optional[conlist(APIReference)] = Field(None, description="A list of the equipment that falls into this category.")
    __properties = ["index", "name", "url", "equipment"]
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> EquipmentCategory:
        """Create an instance of EquipmentCategory from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return EquipmentCategory.parse_obj(obj)

        _obj = EquipmentCategory.parse_obj({
            "index": obj.get("index"),
            "name": obj.get("name"),
            "url": obj.get("url"),
            "equipment": [APIReference.from_dict(_item) for _item in obj.get("equipment")] if obj.get("equipment") is not None else None
        })
        return _obj
