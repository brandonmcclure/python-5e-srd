...
class Rule(BaseModel):
    ...
    desc: Optional[StrictStr] = Field(None, description="Description of the rule.")
    subsections: Optional[conlist(APIReference)] = Field(None, description="List of sections for each subheading underneath the rule in the SRD.")
    __properties = ["index", "name", "url", "desc", "subsections"]
    ...
    def from_dict(cls, obj: dict) -> Rule:
        ...
        _obj = Rule.parse_obj({
            ...
            "url": obj.get("url"),
            "desc": obj.get("desc"),
            "subsections": [APIReference.from_dict(_item) for _item in obj.get("subsections")] if obj.get("subsections") is not None else None
        })
        return _obj
