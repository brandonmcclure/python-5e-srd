...
class APIReferenceList(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> APIReferenceList:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> APIReferenceList:
        """Create an instance of APIReferenceList from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return APIReferenceList.parse_obj(obj)

        _obj = APIReferenceList.parse_obj({
            "count": obj.get("count"),
            "results": [APIReference.from_dict(_item) for _item in obj.get("results")] if obj.get("results") is not None else None
        })
        return _obj
