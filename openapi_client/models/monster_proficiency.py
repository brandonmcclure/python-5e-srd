...
class MonsterProficiency(BaseModel):
    ...
    __properties = ["value", "proficiency"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MonsterProficiency:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of proficiency
        if self.proficiency:
            _dict['proficiency'] = self.proficiency.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> MonsterProficiency:
        """Create an instance of MonsterProficiency from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MonsterProficiency.parse_obj(obj)

        _obj = MonsterProficiency.parse_obj({
            "value": obj.get("value"),
            "proficiency": APIReference.from_dict(obj.get("proficiency")) if obj.get("proficiency") is not None else None
        })
        return _obj
