...
class MonsterSpellcasting(BaseModel):
    ...
    modifier: Optional[Union[StrictFloat, StrictInt]] = None
    components_required: Optional[conlist(StrictStr)] = None
    school: Optional[StrictStr] = None
    slots: Optional[Dict[str, Union[StrictFloat, StrictInt]]] = None
    spells: Optional[conlist(MonsterSpell)] = None
    __properties = ["ability", "dc", "modifier", "components_required", "school", "slots", "spells"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MonsterSpellcasting:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of ability
        if self.ability:
            _dict['ability'] = self.ability.to_dict()
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MonsterSpellcasting:
        """Create an instance of MonsterSpellcasting from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MonsterSpellcasting.parse_obj(obj)

        _obj = MonsterSpellcasting.parse_obj({
            "ability": APIReference.from_dict(obj.get("ability")) if obj.get("ability") is not None else None,
            "dc": obj.get("dc"),
            "modifier": obj.get("modifier"),
            "components_required": obj.get("components_required"),
            "school": obj.get("school"),
            "slots": obj.get("slots"),
            "spells": [MonsterSpell.from_dict(_item) for _item in obj.get("spells")] if obj.get("spells") is not None else None
        })
        return _obj
