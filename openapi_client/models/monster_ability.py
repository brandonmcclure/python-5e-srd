...
class MonsterAbility(BaseModel):
    ...
    __properties = ["charisma", "constitution", "dexterity", "intelligence", "strength", "wisdom"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MonsterAbility:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MonsterAbility:
        """Create an instance of MonsterAbility from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MonsterAbility.parse_obj(obj)

        _obj = MonsterAbility.parse_obj({
