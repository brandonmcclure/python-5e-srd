...
class OptionOneOf6(BaseModel):
    ...
    of: Optional[APIReference] = None
    __properties = ["option_type", "count", "of"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionOneOf6:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of of
        if self.of:
            _dict['of'] = self.of.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> OptionOneOf6:
        """Create an instance of OptionOneOf6 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionOneOf6.parse_obj(obj)

        _obj = OptionOneOf6.parse_obj({
            "option_type": obj.get("option_type"),
            "count": obj.get("count"),
            "of": APIReference.from_dict(obj.get("of")) if obj.get("of") is not None else None
        })
        return _obj
