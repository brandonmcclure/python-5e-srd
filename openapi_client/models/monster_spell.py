...
class MonsterSpell(BaseModel):
    ...
    __properties = ["name", "level", "url", "usage"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MonsterSpell:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MonsterSpell:
        """Create an instance of MonsterSpell from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MonsterSpell.parse_obj(obj)

        _obj = MonsterSpell.parse_obj({
            "name": obj.get("name"),
            "level": obj.get("level"),
            "url": obj.get("url"),
