...
class OptionOneOf3(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionOneOf3:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OptionOneOf3:
        """Create an instance of OptionOneOf3 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionOneOf3.parse_obj(obj)

        _obj = OptionOneOf3.parse_obj({
            "option_type": obj.get("option_type"),
            "choice": Choice.from_dict(obj.get("choice")) if obj.get("choice") is not None else None
        })
        return _obj

from openapi_client.models.choice import Choice
OptionOneOf3.update_forward_refs()
