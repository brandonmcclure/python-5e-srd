...
class Equipment(BaseModel):
    ...
    # data type: Weapon
    anyof_schema_1_validator: Optional[Weapon] = None
    # data type: Armor
    anyof_schema_2_validator: Optional[Armor] = None
    # data type: Gear
    anyof_schema_3_validator: Optional[Gear] = None
    # data type: EquipmentPack
    anyof_schema_4_validator: Optional[EquipmentPack] = None
    if TYPE_CHECKING:
        actual_instance: Union[Armor, EquipmentPack, Gear, Weapon]
    else:
        actual_instance: Any
    any_of_schemas: List[str] = Field(EQUIPMENT_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = Equipment.construct()
        error_messages = []
        # validate data type: Weapon
        if not isinstance(v, Weapon):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Weapon`")
        else:
            return v

        # validate data type: Armor
        if not isinstance(v, Armor):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Armor`")
        else:
            return v

        # validate data type: Gear
        if not isinstance(v, Gear):
            error_messages.append(f"Error! Input type `{type(v)}` is not `Gear`")
        else:
            return v

        # validate data type: EquipmentPack
        if not isinstance(v, EquipmentPack):
            error_messages.append(f"Error! Input type `{type(v)}` is not `EquipmentPack`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in Equipment with anyOf schemas: Armor, EquipmentPack, Gear, Weapon. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> Equipment:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> Equipment:
        """Returns the object represented by the json string"""
        instance = Equipment.construct()
        error_messages = []
        # anyof_schema_1_validator: Optional[Weapon] = None
        try:
            instance.actual_instance = Weapon.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[Armor] = None
        try:
            instance.actual_instance = Armor.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_3_validator: Optional[Gear] = None
        try:
            instance.actual_instance = Gear.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_4_validator: Optional[EquipmentPack] = None
        try:
            instance.actual_instance = EquipmentPack.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into Equipment with anyOf schemas: Armor, EquipmentPack, Gear, Weapon. Details: " + ", ".join(error_messages))
        else:
            return instance
