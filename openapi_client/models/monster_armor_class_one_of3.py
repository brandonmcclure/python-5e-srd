...
class MonsterArmorClassOneOf3(BaseModel):
    ...
    __properties = ["type", "value", "spell", "desc"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('spell'):
            raise ValueError("must be one of enum values ('spell')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MonsterArmorClassOneOf3:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MonsterArmorClassOneOf3:
        """Create an instance of MonsterArmorClassOneOf3 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MonsterArmorClassOneOf3.parse_obj(obj)

        _obj = MonsterArmorClassOneOf3.parse_obj({
            "type": obj.get("type"),
            "value": obj.get("value"),
