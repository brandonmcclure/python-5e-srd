...
class AbilityBonus(BaseModel):
    ...
    __properties = ["bonus", "ability_score"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AbilityBonus:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AbilityBonus:
        """Create an instance of AbilityBonus from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AbilityBonus.parse_obj(obj)

        _obj = AbilityBonus.parse_obj({
            "bonus": obj.get("bonus"),
