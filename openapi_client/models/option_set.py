...
class OptionSet(BaseModel):
    ...
    `Option Set` 
    """
    # data type: OptionSetOneOf
    oneof_schema_1_validator: Optional[OptionSetOneOf] = None
    # data type: OptionSetOneOf1
    oneof_schema_2_validator: Optional[OptionSetOneOf1] = None
    # data type: OptionSetOneOf2
    oneof_schema_3_validator: Optional[OptionSetOneOf2] = None
    if TYPE_CHECKING:
        actual_instance: Union[OptionSetOneOf, OptionSetOneOf1, OptionSetOneOf2]
    else:
        actual_instance: Any
    one_of_schemas: List[str] = Field(OPTIONSET_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = OptionSet.construct()
        error_messages = []
        match = 0
        # validate data type: OptionSetOneOf
        if not isinstance(v, OptionSetOneOf):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OptionSetOneOf`")
        else:
            match += 1
        # validate data type: OptionSetOneOf1
        if not isinstance(v, OptionSetOneOf1):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OptionSetOneOf1`")
        else:
            match += 1
        # validate data type: OptionSetOneOf2
        if not isinstance(v, OptionSetOneOf2):
            error_messages.append(f"Error! Input type `{type(v)}` is not `OptionSetOneOf2`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in OptionSet with oneOf schemas: OptionSetOneOf, OptionSetOneOf1, OptionSetOneOf2. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in OptionSet with oneOf schemas: OptionSetOneOf, OptionSetOneOf1, OptionSetOneOf2. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> OptionSet:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> OptionSet:
        """Returns the object represented by the json string"""
        instance = OptionSet.construct()
        error_messages = []
        match = 0

        # deserialize data into OptionSetOneOf
        try:
            instance.actual_instance = OptionSetOneOf.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into OptionSetOneOf1
        try:
            instance.actual_instance = OptionSetOneOf1.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into OptionSetOneOf2
        try:
            instance.actual_instance = OptionSetOneOf2.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into OptionSet with oneOf schemas: OptionSetOneOf, OptionSetOneOf1, OptionSetOneOf2. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into OptionSet with oneOf schemas: OptionSetOneOf, OptionSetOneOf1, OptionSetOneOf2. Details: " + ", ".join(error_messages))
        else:
            return instance
