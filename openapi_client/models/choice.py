...
class Choice(BaseModel):
    ...
    `Choice` 
    """
    desc: Optional[StrictStr] = Field(None, description="Description of the choice to be made.")
    choose: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="Number of items to pick from the list.")
    type: Optional[StrictStr] = Field(None, description="Type of the resources to choose from.")
    var_from: Optional[OptionSet] = Field(None, alias="from")
    __properties = ["desc", "choose", "type", "from"]
    ...
    def from_dict(cls, obj: dict) -> Choice:
        ...
        _obj = Choice.parse_obj({
            "desc": obj.get("desc"),
            "choose": obj.get("choose"),
            "type": obj.get("type"),
            "var_from": OptionSet.from_dict(obj.get("from")) if obj.get("from") is not None else None
        })
        return _obj
