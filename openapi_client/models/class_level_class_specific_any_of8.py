...
class ClassLevelClassSpecificAnyOf8(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClassLevelClassSpecificAnyOf8:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of sneak_attack
        if self.sneak_attack:
            _dict['sneak_attack'] = self.sneak_attack.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ClassLevelClassSpecificAnyOf8:
        """Create an instance of ClassLevelClassSpecificAnyOf8 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClassLevelClassSpecificAnyOf8.parse_obj(obj)

        _obj = ClassLevelClassSpecificAnyOf8.parse_obj({
            "sneak_attack": ClassLevelClassSpecificAnyOf5MartialArts.from_dict(obj.get("sneak_attack")) if obj.get("sneak_attack") is not None else None
        })
        return _obj
