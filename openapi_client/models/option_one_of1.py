...
class OptionOneOf1(BaseModel):
    """
    OptionOneOf1
    """
    option_type: Optional[StrictStr] = Field(None, description="Type of option; determines other attributes.")
    action_name: Optional[StrictStr] = Field(None, description="The name of the action.")
    count: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="The number of times this action can be repeated if chosen.")
    type: Optional[StrictStr] = Field(None, description="For attack options that can be melee, ranged, abilities, or thrown.")
    __properties = ["option_type", "action_name", "count", "type"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('melee', 'ranged', 'ability', 'magic'):
            raise ValueError("must be one of enum values ('melee', 'ranged', 'ability', 'magic')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionOneOf1:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OptionOneOf1:
        """Create an instance of OptionOneOf1 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionOneOf1.parse_obj(obj)

        _obj = OptionOneOf1.parse_obj({
