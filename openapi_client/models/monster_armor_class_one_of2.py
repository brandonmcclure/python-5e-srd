...
class MonsterArmorClassOneOf2(BaseModel):
    ...
    armor: Optional[conlist(APIReference)] = None
    desc: Optional[StrictStr] = None
    __properties = ["type", "value", "armor", "desc"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('armor'):
            raise ValueError("must be one of enum values ('armor')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MonsterArmorClassOneOf2:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in armor (list)
        _items = []
        if self.armor:
            for _item in self.armor:
                if _item:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MonsterArmorClassOneOf2:
        """Create an instance of MonsterArmorClassOneOf2 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MonsterArmorClassOneOf2.parse_obj(obj)

        _obj = MonsterArmorClassOneOf2.parse_obj({
            "type": obj.get("type"),
            "value": obj.get("value"),
            "armor": [APIReference.from_dict(_item) for _item in obj.get("armor")] if obj.get("armor") is not None else None,
            "desc": obj.get("desc")
        })
        return _obj
