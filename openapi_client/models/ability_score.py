...
class AbilityScore(BaseModel):
    ...
    full_name: Optional[StrictStr] = Field(None, description="Full name of the ability score.")
    skills: Optional[conlist(APIReference)] = Field(None, description="List of skills that use this ability score.")
    __properties = ["index", "name", "url", "desc", "full_name", "skills"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AbilityScore:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AbilityScore:
        """Create an instance of AbilityScore from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AbilityScore.parse_obj(obj)

        _obj = AbilityScore.parse_obj({
            "index": obj.get("index"),
            "name": obj.get("name"),
            "url": obj.get("url"),
            "desc": obj.get("desc"),
            "full_name": obj.get("full_name"),
            "skills": [APIReference.from_dict(_item) for _item in obj.get("skills")] if obj.get("skills") is not None else None
        })
        return _obj
