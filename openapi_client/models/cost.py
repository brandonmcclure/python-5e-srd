...
class Cost(BaseModel):
    ...
    `Cost` 
    """
    quantity: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="Numerical amount of coins.")
    unit: Optional[StrictStr] = Field(None, description="Unit of coinage.")
    __properties = ["quantity", "unit"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Cost:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Cost:
        """Create an instance of Cost from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Cost.parse_obj(obj)
