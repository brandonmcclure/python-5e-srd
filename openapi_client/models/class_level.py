...
class ClassLevel(BaseModel):
    ...
    level: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="The number value for the current level object.")
    ability_score_bonuses: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="Total number of ability score bonuses gained, added from previous levels.")
    prof_bonus: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="Proficiency bonus for this class at the specified level.")
    features: Optional[conlist(APIReference)] = Field(None, description="Features automatically gained at this level.")
    spellcasting: Optional[SubclassLevelSpellcasting] = None
    class_specific: Optional[ClassLevelClassSpecific] = None
    __properties = ["index", "url", "level", "ability_score_bonuses", "prof_bonus", "features", "spellcasting", "class_specific"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClassLevel:
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of class_specific
        if self.class_specific:
            _dict['class_specific'] = self.class_specific.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> ClassLevel:
        """Create an instance of ClassLevel from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClassLevel.parse_obj(obj)

        _obj = ClassLevel.parse_obj({
            "index": obj.get("index"),
            "url": obj.get("url"),
            "level": obj.get("level"),
            "ability_score_bonuses": obj.get("ability_score_bonuses"),
            "prof_bonus": obj.get("prof_bonus"),
            "features": [APIReference.from_dict(_item) for _item in obj.get("features")] if obj.get("features") is not None else None,
            "spellcasting": SubclassLevelSpellcasting.from_dict(obj.get("spellcasting")) if obj.get("spellcasting") is not None else None,
            "class_specific": ClassLevelClassSpecific.from_dict(obj.get("class_specific")) if obj.get("class_specific") is not None else None
        })
        return _obj
