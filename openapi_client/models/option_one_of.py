...
class OptionOneOf(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionOneOf:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OptionOneOf:
        """Create an instance of OptionOneOf from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionOneOf.parse_obj(obj)

        _obj = OptionOneOf.parse_obj({
            "option_type": obj.get("option_type"),
            "item": APIReference.from_dict(obj.get("item")) if obj.get("item") is not None else None
        })
        return _obj
