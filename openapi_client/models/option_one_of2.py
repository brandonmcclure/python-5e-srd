...
class OptionOneOf2(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionOneOf2:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OptionOneOf2:
        """Create an instance of OptionOneOf2 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionOneOf2.parse_obj(obj)

        _obj = OptionOneOf2.parse_obj({
            "option_type": obj.get("option_type"),
            "items": [Option.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj

from openapi_client.models.option import Option
OptionOneOf2.update_forward_refs()
