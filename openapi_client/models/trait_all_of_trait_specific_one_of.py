...
class TraitAllOfTraitSpecificOneOf(BaseModel):
    ...
    """
    damage_type: Optional[APIReference] = Field(None, alias="damage-type")
    breath_weapon: Optional[TraitAllOfTraitSpecificOneOfBreathWeapon] = Field(None, alias="breath-weapon")
    __properties = ["damage-type", "breath-weapon"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TraitAllOfTraitSpecificOneOf:
    ...
    def to_dict(self):
        ...
        if self.damage_type:
            _dict['damage-type'] = self.damage_type.to_dict()
        # override the default output from pydantic by calling `to_dict()` of breath_weapon
        if self.breath_weapon:
            _dict['breath-weapon'] = self.breath_weapon.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> TraitAllOfTraitSpecificOneOf:
        """Create an instance of TraitAllOfTraitSpecificOneOf from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TraitAllOfTraitSpecificOneOf.parse_obj(obj)

        _obj = TraitAllOfTraitSpecificOneOf.parse_obj({
            "damage_type": APIReference.from_dict(obj.get("damage-type")) if obj.get("damage-type") is not None else None,
            "breath_weapon": TraitAllOfTraitSpecificOneOfBreathWeapon.from_dict(obj.get("breath-weapon")) if obj.get("breath-weapon") is not None else None
        })
        return _obj
