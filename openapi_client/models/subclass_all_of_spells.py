...
class SubclassAllOfSpells(BaseModel):
    ...
    __properties = ["prerequisites", "spell"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SubclassAllOfSpells:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SubclassAllOfSpells:
        """Create an instance of SubclassAllOfSpells from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SubclassAllOfSpells.parse_obj(obj)

        _obj = SubclassAllOfSpells.parse_obj({
            "prerequisites": [SpellPrerequisite.from_dict(_item) for _item in obj.get("prerequisites")] if obj.get("prerequisites") is not None else None,
            "spell": APIReference.from_dict(obj.get("spell")) if obj.get("spell") is not None else None
        })
        return _obj
