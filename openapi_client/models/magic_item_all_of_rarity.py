...
class MagicItemAllOfRarity(BaseModel):
    """
    MagicItemAllOfRarity
    """
    name: Optional[StrictStr] = Field(None, description="The rarity of the item.")
    __properties = ["name"]
    ...
    def name_validate_enum(cls, value):
        ...
        if value not in ('Varies', 'Common', 'Uncommon', 'Rare', 'Very Rare', 'Legendary', 'Artifact'):
            raise ValueError("must be one of enum values ('Varies', 'Common', 'Uncommon', 'Rare', 'Very Rare', 'Legendary', 'Artifact')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MagicItemAllOfRarity:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MagicItemAllOfRarity:
        """Create an instance of MagicItemAllOfRarity from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MagicItemAllOfRarity.parse_obj(obj)

        _obj = MagicItemAllOfRarity.parse_obj({
            "name": obj.get("name")
        })
        return _obj
