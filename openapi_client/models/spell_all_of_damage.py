...
class SpellAllOfDamage(BaseModel):
    ...
    SpellAllOfDamage
    """
    # data type: DamageAtCharacterLevel
    oneof_schema_1_validator: Optional[DamageAtCharacterLevel] = None
    # data type: DamageAtSlotLevel
    oneof_schema_2_validator: Optional[DamageAtSlotLevel] = None
    if TYPE_CHECKING:
        actual_instance: Union[DamageAtCharacterLevel, DamageAtSlotLevel]
    else:
        actual_instance: Any
    one_of_schemas: List[str] = Field(SPELLALLOFDAMAGE_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = SpellAllOfDamage.construct()
        error_messages = []
        match = 0
        # validate data type: DamageAtCharacterLevel
        if not isinstance(v, DamageAtCharacterLevel):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DamageAtCharacterLevel`")
        else:
            match += 1
        # validate data type: DamageAtSlotLevel
        if not isinstance(v, DamageAtSlotLevel):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DamageAtSlotLevel`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in SpellAllOfDamage with oneOf schemas: DamageAtCharacterLevel, DamageAtSlotLevel. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in SpellAllOfDamage with oneOf schemas: DamageAtCharacterLevel, DamageAtSlotLevel. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> SpellAllOfDamage:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> SpellAllOfDamage:
        """Returns the object represented by the json string"""
        instance = SpellAllOfDamage.construct()
        error_messages = []
        match = 0

        # deserialize data into DamageAtCharacterLevel
        try:
            instance.actual_instance = DamageAtCharacterLevel.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into DamageAtSlotLevel
        try:
            instance.actual_instance = DamageAtSlotLevel.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into SpellAllOfDamage with oneOf schemas: DamageAtCharacterLevel, DamageAtSlotLevel. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SpellAllOfDamage with oneOf schemas: DamageAtCharacterLevel, DamageAtSlotLevel. Details: " + ", ".join(error_messages))
        else:
            return instance
