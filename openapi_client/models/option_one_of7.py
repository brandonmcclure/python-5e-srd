...
class OptionOneOf7(BaseModel):
    ...
    minimum_score: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="The minimum score required to satisfy the prerequisite.")
    __properties = ["option_type", "ability_score", "minimum_score"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OptionOneOf7:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of ability_score
        if self.ability_score:
            _dict['ability_score'] = self.ability_score.to_dict()
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> OptionOneOf7:
        """Create an instance of OptionOneOf7 from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return OptionOneOf7.parse_obj(obj)

        _obj = OptionOneOf7.parse_obj({
            "option_type": obj.get("option_type"),
            "ability_score": APIReference.from_dict(obj.get("ability_score")) if obj.get("ability_score") is not None else None,
