...
class MonsterUsage(BaseModel):
    """
    MonsterUsage
    """
    type: Optional[StrictStr] = None
    rest_types: Optional[conlist(StrictStr)] = None
    times: Optional[Union[StrictFloat, StrictInt]] = None
    __properties = ["type", "rest_types", "times"]
    ...
    def type_validate_enum(cls, value):
        ...
        if value not in ('at will', 'per day', 'recharge after rest', 'recharge on roll'):
            raise ValueError("must be one of enum values ('at will', 'per day', 'recharge after rest', 'recharge on roll')")
        return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> MonsterUsage:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> MonsterUsage:
        """Create an instance of MonsterUsage from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return MonsterUsage.parse_obj(obj)

        _obj = MonsterUsage.parse_obj({
            "type": obj.get("type"),
            "rest_types": obj.get("rest_types"),
            "times": obj.get("times")
        })
        return _obj
