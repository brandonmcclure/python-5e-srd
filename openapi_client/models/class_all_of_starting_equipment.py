...
class ClassAllOfStartingEquipment(BaseModel):
    ...
    __properties = ["quantity", "equipment"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClassAllOfStartingEquipment:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ClassAllOfStartingEquipment:
        """Create an instance of ClassAllOfStartingEquipment from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClassAllOfStartingEquipment.parse_obj(obj)

        _obj = ClassAllOfStartingEquipment.parse_obj({
            "quantity": obj.get("quantity"),
            "equipment": APIReference.from_dict(obj.get("equipment")) if obj.get("equipment") is not None else None
        })
        return _obj
