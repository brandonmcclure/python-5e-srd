...
class Race(BaseModel):
    ...
    speed: Optional[Union[StrictFloat, StrictInt]] = Field(None, description="Base move speed for this race (in feet per round).")
    ability_bonuses: Optional[conlist(AbilityBonus)] = Field(None, description="Racial bonuses to ability scores.")
    alignment: Optional[StrictStr] = Field(None, description="Flavor description of likely alignments this race takes.")
    age: Optional[StrictStr] = Field(None, description="Flavor description of possible ages for this race.")
    size: Optional[StrictStr] = Field(None, description="Size class of this race.")
    size_description: Optional[StrictStr] = Field(None, description="Flavor description of height and weight for this race.")
    starting_proficiencies: Optional[conlist(APIReference)] = Field(None, description="Starting proficiencies for all new characters of this race.")
    starting_proficiency_options: Optional[Choice] = None
    languages: Optional[conlist(APIReference)] = Field(None, description="Starting languages for all new characters of this race.")
    language_desc: Optional[StrictStr] = Field(None, description="Flavor description of the languages this race knows.")
    traits: Optional[conlist(APIReference)] = Field(None, description="Racial traits that provide benefits to its members.")
    subraces: Optional[conlist(APIReference)] = Field(None, description="All possible subraces that this race includes.")
    __properties = ["index", "name", "url", "speed", "ability_bonuses", "alignment", "age", "size", "size_description", "starting_proficiencies", "starting_proficiency_options", "languages", "language_desc", "traits", "subraces"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Race:
    ...
    def to_dict(self):
        ...
        _dict = self.dict(by_alias=True,
                          ...
                          exclude_none=True)
        # override the default output from pydantic by calling `to_dict()` of each item in ability_bonuses (list)
        _items = []
        if self.ability_bonuses:
            for _item in self.ability_bonuses:
                if _item:
                    _items.append(_item.to_dict())
            _dict['ability_bonuses'] = _items
        ...
        # override the default output from pydantic by calling `to_dict()` of starting_proficiency_options
        if self.starting_proficiency_options:
            _dict['starting_proficiency_options'] = self.starting_proficiency_options.to_dict()
        ...
        if self.traits:
            for _item in self.traits:
                if _item:
                    _items.append(_item.to_dict())
            _dict['traits'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in subraces (list)
        _items = []
        if self.subraces:
            for _item in self.subraces:
                if _item:
                    _items.append(_item.to_dict())
            _dict['subraces'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: dict) -> Race:
        """Create an instance of Race from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Race.parse_obj(obj)
        ...
        _obj = Race.parse_obj({
            ...
            "url": obj.get("url"),
            "speed": obj.get("speed"),
            "ability_bonuses": [AbilityBonus.from_dict(_item) for _item in obj.get("ability_bonuses")] if obj.get("ability_bonuses") is not None else None,
            "alignment": obj.get("alignment"),
            "age": obj.get("age"),
            "size": obj.get("size"),
            "size_description": obj.get("size_description"),
            ...
            "starting_proficiency_options": Choice.from_dict(obj.get("starting_proficiency_options")) if obj.get("starting_proficiency_options") is not None else None,
            "languages": [APIReference.from_dict(_item) for _item in obj.get("languages")] if obj.get("languages") is not None else None,
            "language_desc": obj.get("language_desc"),
            "traits": [APIReference.from_dict(_item) for _item in obj.get("traits")] if obj.get("traits") is not None else None,
            "subraces": [APIReference.from_dict(_item) for _item in obj.get("subraces")] if obj.get("subraces") is not None else None
        })
        return _obj
